
'''
import aiohttp

async def find_image(recipe_name: str):
    async with aiohttp.ClientSession() as session:
        async with session.get(f'https://api.unsplash.com/photos/random?query={recipe_name}') as response:
            if response.status == 200:
                data = await response.json()
                # Assuming data contains image URLs
                image_url = data[0]['urls']['small']
                return image_url
            else:
                return "https://www.cnet.com/a/img/resize/69256d2623afcbaa911f08edc45fb2d3f6a8e172/hub/2023/02/03/afedd3ee-671d-4189-bf39-4f312248fb27/gettyimages-1042132904.jpg?auto=webp&fit=crop&height=675&width=1200"  # Default image if no image found

                '''





import requests
from bs4 import BeautifulSoup

# Default image if no link is found
Not_found_link = 'data:image/png;base64,'

def find_image(recipe_name: str):
    try:
        search_url = f"https://www.google.com/search?q={recipe_name}&tbm=isch"
        d = requests.get(search_url).text
        soup = BeautifulSoup(d, 'html.parser')

        img_tags = soup.find_all('img') 
        img_urls = [img['src'] for img in img_tags if img['src'].startswith("http")]

        return img_urls[0] if img_urls else Not_found_link
    except Exception as e:
        print(f"An error occurred: {e}")
        return Not_found_link













